...
Omega_bar=(1/sqrt(n_cycle)).*sqrt(2.*theta.*((w_COM-delta).^2)./(pi.*(eta_COM.^2))); %Rabi frequency
...
for c=1:N_number_COM
    for cc=1:N_number_Tilt
        ...
        if heattime~=0
            [tm,psi_t]=ode113(@(t,psi) MS_gate_fun_OR_ideal(t,psi,qudit_level,gate_time,n_cycle,eta_COM,w_COM,w_Tilt,w_split,Omega_bar,...
    Cfactor,State_raise,State_lower,State_I,Motion_raise_COM,Motion_lower_COM,...
    Motion_raise_Tilt,Motion_lower_Tilt),[0 heattime*gate_time],psi,options);
            tm1=tm;
            psi_t1=psi_t;
            psi=psi_t(end,:);
            psi=psi.';
        ...
        [tm,psi_t]=ode113(@(t,psi) MS_gate_fun_OR_ideal(t,psi,qudit_level,gate_time,n_cycle,eta_COM,w_COM,w_Tilt,w_split,Omega_bar,...
    Cfactor,State_raise,State_lower,State_I,Motion_raise_COM,Motion_lower_COM,...
    Motion_raise_Tilt,Motion_lower_Tilt),[heattime*gate_time gate_time],psi,options);
        
        tm2=tm;
        psi_t2=psi_t;
        
        %Compute fidelity
        %fidel=sum(abs(kron(Output_qudits',kron(Motion_I_COM,Motion_I_Tilt))*psi_t(end,:).').^2);
